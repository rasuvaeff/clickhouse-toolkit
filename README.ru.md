...
`ClickHouseRawFilter` — это `FilterInterface`, который эммитит сырой SQL-фрагмент для случаев, недоступных типизированным фильтрам. SQL считается доверенным (никогда не из пользовательского ввода); значения попадают в `$params` через `{name:Type}`-плейсхолдеры. Имена изолированы для каждого фильтра: имя, уже занятое в запросе — соседним raw-фильтром, ключом билдера (`p0`, `p1`, …) или обязательным фильтром, — переименовывается в `name_0`, `name_1`, … вместе со своим `{name:Type}`-токеном, так что два raw-фильтра могут оба использовать `{v:UInt64}` и оба значения будут забинжены.
...
Иначе строки, разделяющие граничный ключ, могут быть пропущены. Ключевые столбцы не должны быть nullable. Параметры границы называются `ck0`, `ck1`, …; базовый фильтр с теми же именами переименовывается при слиянии, как любая другая коллизия.
...
Применяет файлы `*.sql` из каталога в порядке имён файлов, записывая каждый применённый файл с **контрольной суммой** в служебную таблицу (по умолчанию `_migrations`).
...
$runner = new ClickHouseMigrationRunner(
    ...
    logger: $logger, // опционально, PSR-3
    placeholders: ['events_table' => 'my_events'], // опционально; заменяет {{events_table}}
    migrationsTable: '_migrations', // опционально; служебная таблица
);

$applied = $runner->run(); // list<string> файлов, применённых этим вызовом
```

#### Выбор служебной таблицы

Имя, отличное от умолчания, нужно в двух случаях:

- **Переход на этот раннер там, где `_migrations` уже существует** с другой
  схемой — например, самописная `(name, applied_at)`. Собственный
  `CREATE TABLE IF NOT EXISTS` раннера находит таблицу и ничего не делает, а
  первое же чтение падает на отсутствующей колонке `checksum`. Это происходит
  *до* чтения любого файла миграции, поэтому починку нельзя поставить самой
  миграцией. Направьте раннер на свежее имя, дайте ему заново применить
  (идемпотентные) миграции, а старую таблицу удалите когда удобно.
- **Два приложения на одной базе ClickHouse** — каждому своя служебная таблица.

Имя подставляется в SQL, а не биндится, поэтому оно обязано быть простым
идентификатором (`/^[A-Za-z_]\w*\z/`); всё остальное конструктор отвергает
через `InvalidArgumentException`. Форма с базой, `analytics._migrations`, тоже
отвергается: обратные кавычки оборачивают строку целиком, и получилось бы одно
имя таблицы с точкой внутри. Выбирайте базу на стороне клиента.

Контрольная сумма считается по **подставленному** SQL, а не по сырому файлу.
Отсюда два следствия, оба намеренные:

- перевод поставляемой миграции с литерального имени на `{{placeholder}}`
  незаметен для инсталляций на значении по умолчанию — подставленный текст
  байт в байт совпадает с тем, что они применяли;
- изменение значения **после** применения миграции сообщается как расхождение,
  а не создаёт молча вторую таблицу. Создайте новую таблицу сами либо удалите
  строку из служебной таблицы и перезапустите.

Значения плейсхолдеров подставляются в DDL дословно. Они приходят из вашей
конфигурации, а не от пользователя; если источник может быть иным — проверяйте
через `Identifier::assertPlain()`.

Таблица трекинга (создаётся автоматически, имя задаёт `$migrationsTable`):
...
- **`ClickHouseMigrationRunner::status()`** сообщает состояние каждого файла миграции относительно служебной таблицы.
...
| `Pending` | Файл существует, ещё не записан в служебную таблицу. |
| `Missing` | Записан в служебной таблице, но исходный файл удалён. |
